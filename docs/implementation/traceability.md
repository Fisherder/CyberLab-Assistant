...
| FR-CONTENT-001 | `content/challenges/web-sqli-auth/manifest.yaml`，`content/challenge-blueprints/authoritative-catalog.yaml`，`packages/contracts/json-schema/challenge.schema.json`，`POST /api/v1/challenge-registry/import-local`，`POST /api/v1/challenge-registry/import-blueprints` | 题目 fixture 通过 JSON Schema；API 测试覆盖本地 Challenge 包导入、内容验证、Registry 查询、150 条权威题型蓝图导入和 Web/REVERSE/PWN 各 50 条数量门禁。 | 部分完成：蓝图不是完整可发布题目包；生产内容 CI、镜像签名、pgvector/OpenSearch 和大规模质量排序仍未完成。 |
...
| 权威题型蓝图库 | `tools/generate_authoritative_blueprint_catalog.py`，`content/challenge-blueprints/authoritative-catalog.yaml`，`docs/development/authoritative-blueprint-catalog.md` | 生成脚本输出 150 条蓝图，三类各 50；测试覆盖导入、验证报告、检索组合和无候选生成；文档记录来源策略、合规边界、导入流程、组合逻辑和后续扩展规范。 | 已验证本地切片；完整题目物化、向量检索和生产内容质量门禁仍待后续实现。 |
