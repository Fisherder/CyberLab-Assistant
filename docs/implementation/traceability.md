...
| FR-CONTENT-001 | `content/challenges/web-sqli-auth/manifest.yaml`，`content/challenge-blueprints/authoritative-catalog.yaml`，`packages/contracts/json-schema/challenge.schema.json`，`POST /api/v1/challenge-registry/import-local`，`POST /api/v1/challenge-registry/import-blueprints` | 题目 fixture 通过 JSON Schema；API 测试覆盖本地 Challenge 包导入、内容验证、Registry 查询、300 条权威题型蓝图导入和 Web/REVERSE/PWN/CRYPTO/FORENSICS/MISC 各 50 条数量门禁。 | 部分完成：蓝图不是完整可发布题目包；生产内容 CI、镜像签名、pgvector/OpenSearch 和大规模质量排序仍未完成。 |
...
| FR-AUTHOR-002 | `GET /api/v1/challenge-drafts/{id}/candidates`，`GET /api/v1/challenge-registry`，`services/api/src/cla/challenge_catalog.py` | 测试覆盖硬过滤、候选理由、REMOTE_DESKTOP 冲突拒绝、BM25 风格全文检索、Registry 列表、对象资产展示、权威蓝图检索、`compositionPlan`、逆向分类和 source-backed-blueprint 信号。 | 部分完成：pgvector/OpenSearch 语义向量检索和大规模教学质量排序未完成。 |
| FR-AUTHOR-003 | `POST /api/v1/challenge-drafts/{id}/generate-version`，`POST /api/v1/challenge-drafts/{id}/generate-custom-package`，materialize 与 approve API | 测试覆盖模型生成题目版本草稿、Rubric 草稿、`PENDING_APPROVAL` 版本、验证报告、审批后发布、学生拒绝、审计、无候选时生成定制靶场 tar 包草稿和生成资产内容；本机 live smoke 已验证 `generatedBy=model`、验证报告 `PASS`、教师审批后 `published=true`。 | 部分完成：定制包仍是教师审核草稿；Agent 能力 broker、Temporal 等待审批、内容 CI 物化执行和多人审核策略未完成。 |
...
| 工程文档与开发规范 | `README.md`，`docs/development/developer-guide.md`，`docs/development/architecture.md`，`docs/development/security.md`，`docs/development/testing.md`，`docs/development/git.md`，`docs/development/content-authoring.md`，`docs/development/authoritative-blueprint-catalog.md`，`docs/runbooks/local-development.md` | 文档覆盖项目定位、模块职责、核心链路、安全边界、模型接入、权威题型蓝图库、题库 Registry、对象资产、测试矩阵、Git 协作、内容开发和本地运行；命名扫描和 Web 生成注释检查已通过。 | 已完成本轮文档交付。 |
| 权威题型蓝图库 | `tools/generate_authoritative_blueprint_catalog.py`，`content/challenge-blueprints/authoritative-catalog.yaml`，`docs/development/authoritative-blueprint-catalog.md` | 生成脚本输出 300 条蓝图，六类各 50；测试覆盖导入、验证报告、检索组合、60 场景常见安全题型矩阵和无候选生成；文档记录来源策略、合规边界、导入流程、组合逻辑和后续扩展规范。 | 已验证本地切片；完整题目物化、向量检索和生产内容质量门禁仍待后续实现。 |
...
| P1 纵向核心 | 教师建作业/题库发布 → 学生 Attempt → Session/Ticket → Gateway → Event/Transcript → Oracle → Grade → Appeal → Monitor | API `80 passed, 1 skipped`；Go sessionwire/gateway/sessiond/controller 全部通过；Web build/typecheck 通过；本机浏览器已验证登录页样式、学生题库、Attempt、Lab Ready、xterm.js、命令回显、L1 提示、提交、成绩页、申诉、教师题库、教师验证报告和 live monitor。 | 部分完成：真实 Compose target/Oracle PASS、MinIO/Redis live 和生产集群 E2E 被 Docker/Kubernetes 环境不可用阻塞。 |
