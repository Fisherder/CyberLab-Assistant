...
| FR-CONTENT-003 | `services/api/src/cla/content_validation.py`，`Taskfile.yml`，教师验证报告页面 | 测试覆盖 schema/rubric、目标 HTTP smoke、Oracle 正负例、WARN/BLOCK 报告、教师审批门禁和页面构建。 | 部分完成：真实 OCI 构建、Trivy/SBOM、报告签名、Temporal 发布流程未完成。 |
...
| FR-TERM-001 | 终端票据 API、内部 consume API、路由注册、`apps/web/components/TerminalWorkbench.tsx`、Gateway | API 测试覆盖 60 秒票据、单次消费、route/endpoint 不泄露、reset/unregister/revoke 后旧票据拒绝；Go Gateway 测试覆盖控制面消费。 | 部分完成：浏览器级 E2E 未执行。 |
| FR-TERM-002 | Gateway 二进制协议、sessionwire、sessiond、重连缓冲、ACK 背压、Prometheus metrics | Go 测试覆盖 STDIN/STDOUT、resize、heartbeat、ACK、60 秒/1 MiB replay、Redis replay、背压窗口、Gateway metrics 和异步录制。 | 部分完成：live Redis/Compose 断线重连、指标 scrape/告警和浏览器断线 E2E 未执行。 |
...
| FR-MONITOR-001 | 教师 live monitor API 和页面 | 测试覆盖教师 RBAC、READY 统计、卡住统计、提示摘要、资源/安全告警计数和无原始终端文本泄露。 | 部分完成：生产资源 telemetry、SSE 和浏览器 E2E 未完成。 |
...
| FR-GRADE-003 | GradeRevision、CriterionResult、Appeal API、学生成绩页 | API 测试覆盖证据页数据、跨学生拒绝、申诉 criterion 校验、教师 override 生成新 revision；Web 构建覆盖页面。 | 部分完成：教师复核 UI 未完成。 |
...
| P1 纵向核心 | 教师建作业 → 学生 Attempt → Session/Ticket → Gateway → Event/Transcript → Oracle → Grade → Appeal → Monitor | API `60 passed, 1 skipped`；Go sessionwire/gateway/sessiond/controller 全部通过；Web build 和 typecheck 通过。 | 部分完成：真实 Compose/浏览器/集群 E2E 被 Docker daemon 不可用阻塞。 |

## 未标记完成的关键缺口

- Docker Compose 真实 lab 启动未运行。
- 浏览器连接 Gateway、断线重连和 Oracle PASS 的端到端路径未运行。
