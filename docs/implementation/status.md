...
- Web：学生工作台、成绩证据页、教师验证报告页、教师 live monitor 页面可以构建并通过类型检查。
...
# 结果：60 passed, 1 skipped in 3.11s
...
8. 教师可查看 live monitor 和处理申诉 API。

## 已知限制

- Docker daemon 当前不可用，因此真实 `docker compose build/up`、浏览器终端 E2E 和本地 MinIO/Redis live smoke 未运行。
- Kubernetes controller 目前通过 fake client、静态 Helm/CRD 和单元测试验证；真实集群 apply、NetworkPolicy、gVisor/Kata、节点故障和 orphan cleanup 未运行。
...
2. 运行浏览器 E2E：登录、Attempt、Lab Ready、xterm.js 连接、curl、Oracle PASS、提交、成绩页和申诉。
