...
- 内容与出题：支持本地 Challenge 包导入 Registry、300 条权威题型蓝图导入、对象资产记录、硬约束过滤 + BM25 风格全文检索、候选组合计划、教师 Brief 解析、模型辅助生成题目版本草稿、无候选时生成定制靶场代码包草稿、验证报告和教师审批发布。
- 课堂题库：教师可把已发布的 ChallengeVersion 创建为课程题库条目，设置开始/截止时间，执行发布、下架、修改、删除、回收站和恢复；教师端提供题库侧边栏、浮层详情和独立 Agent 创建页；学生只看到本课程已发布题目，并从题库获取或销毁本人隔离环境。
...
- Web 前端：登录/注册页、学生题库、学生终端工作台、成绩证据页、教师题库、教师 Agent 创建页、教师题目验证报告页、教师 live monitor 页面。
...
- pgvector/OpenSearch 级别的语义向量检索和大规模题库质量排序，当前本地实现为硬约束过滤加 BM25 风格全文检索，并已保留向量分数字段。
...
11. [docs/development/authoritative-blueprint-catalog.md](/Users/fisherder/Desktop/研究生/Security_Class_Tool/docs/development/authoritative-blueprint-catalog.md)：300 条权威题型蓝图、导入、检索组合、60 场景题型矩阵和无候选定制生成流程。
12. [docs/development/git.md](/Users/fisherder/Desktop/研究生/Security_Class_Tool/docs/development/git.md)：Git 初始化、提交、远程推送和协作规范。
13. [docs/runbooks/local-development.md](/Users/fisherder/Desktop/研究生/Security_Class_Tool/docs/runbooks/local-development.md)：本地运行、本地账号登录、开发 token 和常见故障。
...
├── content/challenge-blueprints/      # 权威题型蓝图库，当前 Web/逆向/Pwn 各 50 条
...
| `services/agent-runtime` | 受限 Agent Harness 边界包；当前 API 内已实现 OpenAI-compatible Provider Adapter，用于 Brief 解析和版本草稿 | 不持有基础设施管理员能力，不直接写业务数据库，不获得 Shell/SQL/Docker/Kubernetes 工具 |
| `apps/web` | 学生题库、学生终端、提示、提交、成绩证据、教师题库、教师验证和 live monitor 页面 | 不保存基础设施凭据，不展示 `route_ref`、容器 IP 或原始敏感终端内容 |
...
2. API 优先调用 `.env` 配置的 OpenAI-compatible 模型解析 CourseIntent；模型不可用或输出不合格时回退到规则解析。
3. 候选题按类别、目标、难度、时间、workspace 类型、隔离等级和允许工具进行硬过滤，并叠加 BM25 风格全文得分。
4. 教师选择候选题，模型可生成题目版本草稿、rubric 草稿和教师审核清单。
5. API materialize 出不可变 `PENDING_APPROVAL` ChallengeVersion，并写入题目资产对象引用和验证记录。
6. 内容验证产生报告，包含 Schema、Rubric、目标服务 smoke、Oracle 正负例、策略和审批门禁。
7. 教师审批后 ChallengeVersion 进入可发布状态。

模型只参与 Brief 解析和草稿生成，不能直接发布、部署、评分或操作基础设施。每次模型调用都会写入 `AgentRun`，不记录模型 API Key。
...
| `CLA_LOCAL_TARGET_BASE_URL` | 本地 Web 靶标地址，提交时 Oracle 检查会访问它 | `http://127.0.0.1:18080` |
| `CLA_LOCAL_TARGET_SESSION_KEY` | 本地 target 与 API 共享的 Oracle 会话密钥 | `dev-session-key` |
...
| `CLA_CHALLENGE_ARTIFACT_STORAGE_BACKEND` | 题目资产对象后端 | `local` 或 `s3` |
| `CLA_CHALLENGE_ARTIFACT_OBJECT_ROOT` | 本地题目资产对象目录 | `/tmp/cla-challenge-artifacts` |
| `CLA_AGENT_RUNTIME_ENABLED` | 是否启用模型辅助出题 | `false` |
| `CLA_MODEL_PROVIDER` | 模型供应商适配器 | `openai-compatible` |
| `CLA_MODEL_BASE_URL` | 模型 API 基础地址 | 例如 `https://你的地址/v1` |
| `CLA_MODEL_NAME` | 模型名称 | `deepseek-v4-flash` |
| `CLA_MODEL_API_KEY` | 模型 API Key | 只写 `.env`，不要提交 |
| `CLA_REMOTE_DESKTOP_ENABLED` | GUI 预留开关 | 必须为 `false` |
| `CLA_SIMULATED_WORKSPACE_ENABLED` | 模拟 workspace 预留开关 | 必须为 `false` |

启用 DeepSeek v4 Flash 兼容模型的本地 `.env` 示例：
...
CLA_MODEL_TIMEOUT_SECONDS=30
CLA_MODEL_MAX_TOKENS=1200
CLA_MODEL_TEMPERATURE=0
```

如果你的模型接口不是 OpenAI-compatible `/chat/completions` 格式，需要新增 Provider Adapter，而不是把任意 HTTP 能力暴露给 Agent。
...
脚本会在默认 tmux socket 中创建 `cla` 会话，并启动 target、sessiond、API、Gateway 和 Web。学生终端使用专用工作区 `/private/tmp/cla-local-workspace/web-sqli-auth`，不会暴露宿主 `/tmp`。
...
可以在登录页注册学生或教师账号。注册学生账号后进入学生题库；注册教师账号后进入教师题库。生产环境仍建议接入学校 OIDC，并由管理员审核教师身份。

教师题库 Registry：

```text
http://localhost:3000/teacher/challenges/registry
```

该页面可以导入 `content/challenges/*` 下的本地题目包、搜索题库、输入 Brief、查看候选、生成模型辅助版本草稿、打开验证报告并审批发布。
