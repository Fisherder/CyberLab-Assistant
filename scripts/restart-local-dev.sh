...
TARGET_SESSION_KEY="${CLA_LOCAL_TARGET_SESSION_KEY:-dev-session-key}"
...
tmux_cmd new-session -d -s "$SESSION" -n target -c "$ROOT" \
  "env TARGET_PORT=18080 TARGET_SESSION_KEY='$TARGET_SESSION_KEY' '$PYTHON_BIN' content/challenges/web-sqli-auth/target/server.py"
